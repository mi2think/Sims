...
class TranslationTransformation : public App<HWWindow>
{
public:
	...
	virtual void OnRender(const Timestep&)
	{
		if (renderer_)
		{
			...
			Matrix44f mat;
			mat.Identity();
			mat.SetTranslation(Vector3f(sinf(scale), 0, 0));
	...
	void SetupProg()
	{
		...
		VS->SetSourceFromFile("../res/06_TranslationTransformation/ogl/vs.glsl");
		PS->SetSourceFromFile("../res/06_TranslationTransformation/ogl/ps.glsl");
#elif SIMS_SDK_IMPL_D3D9
		VS->SetSourceFromFile("../res/06_TranslationTransformation/dx9/vs.hlsl");
		PS->SetSourceFromFile("../res/06_TranslationTransformation/dx9/ps.hlsl");
#endif
...
int main()
{
	TranslationTransformation app;
	app.Create(640, 480, "Translation Transformation");
	app.Loop();
	return 0;
}
