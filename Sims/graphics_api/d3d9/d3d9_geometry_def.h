/********************************************************************
	...
	filename: 	D:\Code\Sims\Sims\graphics_api\d3d9\d3d9_geometry_def.h
	file path:	D:\Code\Sims\Sims\graphics_api\d3d9
	file base:	d3d9_geometry_def
	file ext:	h
	...
	namespace d3d9
	{
		...
		public:
			...
			void Init(float width, float height, float z, const Matrix44f& m)
			{
				...
				CHECK_HR = d3d9::g_pD3DD->CreateVertexBuffer(sizeof(vertices),
					D3DUSAGE_WRITEONLY,
					T::FVF,
					D3DPOOL_MANAGED,
					&vb_,
					0);
				T* v = nullptr;
				CHECK_HR = vb_->Lock(0, 0, (void**)&v, 0);
			...
			void Draw(D3DXMATRIX* world, D3DMATERIAL9* mtrl, IDirect3DTexture9* tex)
			{
				...
				if (tex)
					CHECK_HR = d3d9::g_pD3DD->SetTexture(0, tex);

				CHECK_HR = d3d9::g_pD3DD->SetStreamSource(0, vb_, 0, sizeof(T));
				CHECK_HR = d3d9::g_pD3DD->SetFVF(T::FVF);
				CHECK_HR = d3d9::g_pD3DD->DrawPrimitive(D3DPT_TRIANGLELIST, 0, 2);
			}
		...
		public:
			...
			void Init(float width, float height, uint32 slices, uint32 stacks, const Matrix44f& m)
			{
				...
				CHECK_HR = d3d9::g_pD3DD->CreateVertexBuffer(sizeof(T) * vertexNum,
					D3DUSAGE_WRITEONLY,
					T::FVF,
					D3DPOOL_MANAGED,
					&vb_,
				...
				CHECK_HR = d3d9::g_pD3DD->CreateIndexBuffer(sizeof(uint16) * indexNum,
					D3DUSAGE_WRITEONLY,
					D3DFMT_INDEX16,
					D3DPOOL_MANAGED,
					&ib_,
					0);
				uint16* w = nullptr;
				CHECK_HR = ib_->Lock(0, 0, (void**)&w, 0);
			...
			void Draw(D3DXMATRIX* world, D3DMATERIAL9* mtrl, IDirect3DTexture9* tex)
			{
				...
				CHECK_HR = d3d9::g_pD3DD->SetStreamSource(0, vb_, 0, sizeof(T));
				CHECK_HR = d3d9::g_pD3DD->SetFVF(T::FVF);
				CHECK_HR = d3d9::g_pD3DD->SetIndices(ib_);
				CHECK_HR = d3d9::g_pD3DD->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 0, vertexNum_, 0, triNum_);
			}
		...
		public:
			...
			void Draw(D3DXMATRIX* world, D3DMATERIAL9* mtrl, IDirect3DTexture9* tex)
			{
				...
				CHECK_HR = d3d9::g_pD3DD->SetFVF(T::FVF);
				CHECK_HR = d3d9::g_pD3DD->SetIndices(ib_);
				CHECK_HR = d3d9::g_pD3DD->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 0, vertexNum_, 0, 12);
			}
		...
		public:
			...
			void Init(float width, float height, float depth, const Matrix44f& m)
			{
				...
				CHECK_HR = vb_->Lock(0, 0, (void**)&v, 0);
				memcpy(v, &vertices[0], sizeof(vertices));
				CHECK_HR = vbWall_->Unlock();

				CHECK_HR = d3d9::g_pD3DD->CreateIndexBuffer(sizeof(indices),
					D3DUSAGE_WRITEONLY,
					D3DFMT_INDEX16,
					D3DPOOL_MANAGED,
					&ib_,
			...
			void Draw(D3DXMATRIX* world, D3DMATERIAL9* mtrl, IDirect3DTexture9* tex)
			{
				...
				CHECK_HR = d3d9::g_pD3DD->SetFVF(T::FVF);
				CHECK_HR = d3d9::g_pD3DD->SetIndices(ib_);
				CHECK_HR = d3d9::g_pD3DD->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 0, 14, 0, 12);
			}
